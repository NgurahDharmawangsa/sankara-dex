...
function handleValidation(messages) {
    ...
    for (let i in messages) {
        for (let t in messages[i]) {
            if(i == "proof_of_payment" || i == "certificate_file") {
                $('[name=' + i + ']').addClass('is-invalid').parent().parent().append('<div class="text-left custom-invalid-feedback order-3">' + messages[i][t] + '</div>');
            } else {
